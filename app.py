...
st.set_page_config(page_title="BingeSense", page_icon="🎬", layout="wide")
...
.stApp {
    background:
        radial-gradient(circle at top left, rgba(226, 232, 240, 0.55), transparent 28%),
        linear-gradient(180deg, #f6efe7 0%, #fbfaf7 46%, #f3f4f6 100%);
}
.block-container {
    padding-top: 1rem;
    padding-bottom: 2rem;
...
.hero {
    ...
    border-radius: 24px;
    background: linear-gradient(135deg, rgba(15, 23, 42, 0.95), rgba(180, 83, 9, 0.85));
    color: white;
    margin-bottom: 1rem;
    ...
    for row in range(rows):
        ...
        for col in range(cols):
            ...
            with colset[col]:
                ...
                if st.button("Open", key=f"{key_prefix}_{row}_{col}_{idx}_{tmdb_id}"):
                    if tmdb_id:
...
# =============================
with st.sidebar:
    st.markdown("## BingeSense")
    st.caption("Discover something worth watching.")
    if st.button("Home"):
        goto_home()
...
st.markdown(
    ...
    <div class="hero">
        <h1>BingeSense</h1>
        <p>Browse live movie feeds, search instantly, and explore similar picks powered by TMDb and a local recommendation model.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
st.markdown(
    "<div class='small-muted'>Search a title, open the movie page, and jump into recommendations in a few clicks.</div>",
    unsafe_allow_html=True,
)
st.divider()
...
if st.session_state.view == "home":
    typed = st.text_input(
        ...
        placeholder="Try: avengers, batman, interstellar...",
    )
    ...
    st.markdown(
        f"<div class='section-label'>{home_category.replace('_', ' ').title()} Picks</div>",
        unsafe_allow_html=True,
    )
...
# ==========================================================
    ...
    with left_head:
        st.markdown(
            "<div class='section-label'>Movie Details</div>", unsafe_allow_html=True
        )
    with right_head:
        if st.button("Back to Home"):
            goto_home()
    ...
    if title:
        ...
        if not err2 and bundle:
            ...
            st.info("Showing genre recommendations instead.")
